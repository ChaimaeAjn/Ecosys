    ...
    public class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    private void CheckCollisions()
    {
        ...
        foreach (var herbivore in _herbivores.ToList())
        {
            ...
            {
                if (DistanceBetween(herbivore, plante) < COLLISION_DISTANCE)
                {
                    herbivore.EatPlante();
        ...
        foreach (var carnivore in _carnivores.ToList())
        {
            ...
            foreach (var herbivore in _herbivores.ToList())
            {
                ...
                if (DistanceBetween(carnivore, herbivore) < COLLISION_DISTANCE)
                {
                    ...
                    // Créer un nouvel herbivore ailleurs
                    var nouvelHerbivore = new Herbivore(
                        _random.Next(0, (int)CanvasWidth),
                        _random.Next(0, (int)CanvasHeight),
                        CanvasWidth,
                        CanvasHeight
                    );
                    _herbivores.Add(nouvelHerbivore);
                    EcosysObjects.Add(nouvelHerbivore);
                }
            }
        }
    }
